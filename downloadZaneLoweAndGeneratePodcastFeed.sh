...
get_iplayer "$searchString" --get --type=radio --output $showDirectory --thumbnail --fileprefix="<pid>" --metadata=generic --modes=flashhd,flashvhigh,flashhigh,flashstd,flashnormal,flashlow,flashaachigh,flashaacstd,flashaudio,flashaaclow,wma
...
shopt -s nullglob
cp -u "$showDirectory"/*.{m4a,mp3,jpg} $mediaFileDirectoryPath
